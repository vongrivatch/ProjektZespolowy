...
function AnimalTestPage() {
  ...
  const getRandomIndex = () => {
    return Math.floor(Math.random() * animalsFlashcards.length);
  };

  const checkAnswer = () => {
    if (userAnswer.toLowerCase() === animalsFlashcards[currentCardIndex].answer.toLowerCase()) {
      setIsCorrect(true);
    } else {
      setIsCorrect(false);
    }
  ...
  const currentCard = animalsFlashcards[currentCardIndex];

  return (
    <div className="test-page">
      <h1>Test Your Knowledge in Animals</h1>
      <Link to="/">Home</Link>
      <div className="card">
